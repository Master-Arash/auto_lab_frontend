...
export default function EditReferenceStandardPage() {
  const { id } = useParams();
  const navigate = useNavigate();
  const { t } = useTranslation();
  const back_url = "/reference-standards";
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        let rawData = (
          await api.get("/get-reference-standards", { params: { id } })
        ).data;
  ...
  const onSubmit = async (data) => {
    try {
      ...
      payload["id"] = id;
      await api.put("/edit-reference-standard/", payload);
      navigate(back_url, { replace: true });
    } catch (err) {
